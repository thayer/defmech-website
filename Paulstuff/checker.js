...
// Checks to make sure that entry has no letters, other than one 'E' and properly placed '-' and '.'
 // if there's an "e", convert it to "E" and recall function.
   if (entry.indexOf("e")!=-1) {
	return noLetters(entry.substring(0,entry.indexOf("e")) + "E" + entry.substring(entry.indexOf("e")+1,entry.length));
   }
...
// Assume: entry has one or zeros E's or e's. lparen is either '(' or '['. rparen is either ')' or ']'. Min and max are valid numbers.
// Return: True if entry is within the interval specified, false otherwise.

// if there's an "e", convert it to "E" and recall function
   if (entry.indexOf("e")!=-1) {
	return interval(entry.substring(0,entry.indexOf("e")) + "E" + entry.substring(entry.indexOf("e")+1,entry.length),lparen,min,max,rparen);
   }

   if ((min.indexOf("e")!=-1) || (max.indexOf("e")!=-1)) {
	alert ("Error - min and max in call to interval must contain 'E', not 'e'");
	return false;
   }	

   if (entry.indexOf("e")!=-1) {
	//entry[entry.indexOf("e")] = "E";
	//return interval(entry,lparen,min,max,rparen);
   }
